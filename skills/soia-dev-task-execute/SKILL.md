...
description: 执行任意工程任务的通用闭环：定义边界、实施最小改动、验证、独立复核与回执。适用于代码、配置、文档和维护任务。触发：「把这件事做完」「按闭环执行」「有风险的维护」
version: 1.0.4
...
- Provider 凭据必须使用官方登录态或系统凭据库；普通 `config.yml` 只允许非秘密偏好和路径指针。
- 回执记录改动路径、命令、退出状态和必要摘要；不回显秘密、无关源码或完整客户输入。
